...
- **Tenant-aware formatters.** Manual `useDataGrid` does **not** auto-read `<DocyrusTenantProvider>` context (that's only `useDocyrusDataGrid`). Read the formatters yourself from `useDateFormat()` / `useNumberFormat()` (or `useDocyrusTenant()`) and pass `formatDate`, `formatDateTime`, `formatNumber` through `useDataGrid({ meta: { ... } })`. Cells read them from `tableMeta`.
